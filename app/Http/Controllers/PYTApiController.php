...
class PYTApiController extends Controller
{
    public $pyt = 3;
    ...
    public function categories()
    {
        ...
        // })->get();
        $categories = Category::where('storeid', $this->pyt)->get();
    ...
    public function products($category_id)
    {
        ...
        ->where('ps.storeid', $this->pyt)
        ->select('c.name as category_name', 'p.name', 'p.price', 'p.images', 'ps.sub_category_id', 'ps.category_id', 'ps.product_id')
        ->get();
        return $data;

    }

    public function products_special($category_id, $sub_categoryid)
    {
        $data = DB::table('products as p')
        ->join('product_subcategories as ps', 'ps.product_id', '=', 'p.id')
        ->join('subcategories as s', 's.id', '=', 'ps.sub_category_id')
    ...
    public function allSubCategories()
    {
        $query = DB::table('subcategories as s')
                ->join('categories as c', 'c.id', '=', 's.category_id')
                ->when($this->pyt, function($query) {
                    return $query->where('s.storeid', $this->pyt);
                })
                ->select('s.image', 's.subcategory_name', 'c.name as category_name', 's.id', 'c.id as category_id', 'c.banner_image', 'c.image as category_image', 'c.description')
                ->get();
        return $query;
    ...
    public function allProducts()
    {
        ...
        ->join('categories as c', 'c.id', '=', 'ps.category_id')
        ->when($this->pyt, function($query) {
            return $query->where('ps.storeid', $this->pyt);
        })
        ->select('c.name as category_name', 'p.name', 'p.price', 's.subcategory_name', 'p.images', 'ps.sub_category_id', 'ps.category_id', 'ps.product_id', 'p.details', 'p.video')
        ->get();
        return $data;
    }
}
