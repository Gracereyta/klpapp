...
class LifestyleApiController extends Controller
{
    ...
    public function categories()
    {
        ...
        // })->get();
        $categories = Category::where('storeid', $this->ftv)->get();
    ...
    public function products($category_id)
    {
        ...
        ->where('ps.storeid', $this->ftv)
        ->select('c.name as category_name', 'p.name', 'p.price', 'p.images', 'ps.sub_category_id', 'ps.category_id', 'ps.product_id', 'p.video')
        ->get();
        return $data;
    ...
    public function allSubCategories()
    {
        $query = DB::table('subcategories as s')
                ->join('categories as c', 'c.id', '=', 's.category_id')
                ->when($this->ftv, function($query) {
                    return $query->where('s.storeid', $this->ftv);
                })
                ->select('s.image', 's.subcategory_name', 'c.name as category_name', 's.id', 'c.id as category_id', 'c.banner_image', 'c.image as category_image', 'c.description')
                ->get();
        return $query;
    ...
    public function allProducts()
    {
        ...
        ->join('categories as c', 'c.id', '=', 'ps.category_id')
        ->when($this->ftv, function($query) {
            return $query->where('ps.storeid', $this->ftv);
        })
